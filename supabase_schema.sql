...
--
-- Name: create_branch_and_update_tenant(jsonb); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.create_branch_and_update_tenant(branch_data jsonb) RETURNS TABLE(id uuid, tenant_id uuid, name text, latitude double precision, longitude double precision, address text, radius_meters numeric, created_at timestamp with time zone, updated_at timestamp with time zone, details jsonb)
    LANGUAGE plpgsql
    AS $$
DECLARE
    new_branch_record branches%ROWTYPE; -- متغير لحفظ بيانات الفرع الجديد
    p_tenant_id uuid;
BEGIN
    -- استخراج 'tenant_id' من البيانات المدخلة
    p_tenant_id := (branch_data->>'tenant_id')::uuid;

    -- 1. إضافة الفرع الجديد إلى جدول 'branches'
    ...
    VALUES (
        ...
        branch_data->>'name',
        branch_data->>'address',
        (branch_data->>'latitude')::double precision,
        (branch_data->>'longitude')::double precision,
        (branch_data->>'radius_meters')::numeric, -- تم التعديل ليطابق نوع الحقل
        (branch_data->>'details')::jsonb
    )
    RETURNING * INTO new_branch_record; -- تخزين بيانات الفرع المُنشأ

    -- 2. زيادة عدّاد الفروع 'current_branches' في جدول 'tenants'
    UPDATE public.tenants
    SET current_branches = current_branches + 1,
        updated_at = now() -- تحديث حقل 'updated_at' أيضًا
    WHERE tenants.id = p_tenant_id;

    -- 3. إرجاع بيانات الفرع الجديد الذي تم إنشاؤه
    RETURN QUERY SELECT 
        new_branch_record.id,
        new_branch_record.tenant_id,
        new_branch_record.name,
        new_branch_record.latitude,
        new_branch_record.longitude,
        new_branch_record.address,
        new_branch_record.radius_meters,
        new_branch_record.created_at,
        new_branch_record.updated_at,
        new_branch_record.details;
END;
$$;


--
-- Name: create_user_and_update_tenant(jsonb); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.create_user_and_update_tenant(user_data jsonb) RETURNS TABLE(id uuid, tenant_id uuid, branch_id uuid, email text, role text, name text, phone text, avatar text, base_salary numeric, allowances jsonb, deductions jsonb, work_schedule jsonb, is_active boolean, created_at timestamp with time zone, updated_at timestamp with time zone)
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
DECLARE
    new_user_record public.users%ROWTYPE;
    p_tenant_id uuid;
    p_user_id uuid;
BEGIN
    -- Extract IDs
    p_tenant_id := (user_data->>'tenant_id')::uuid;
    p_user_id := (user_data->>'id')::uuid;
    ...
    VALUES (
        ...
        user_data->>'phone',
        user_data->>'role',
        (user_data->>'branch_id')::uuid,
        user_data->>'avatar',
        (user_data->>'base_salary')::numeric,
        (user_data->>'allowances')::jsonb,
        (user_data->>'deductions')::jsonb,
        (user_data->>'work_schedule')::jsonb,
        (user_data->>'is_active')::boolean
    )
    ON CONFLICT (id) DO NOTHING   -- ⚠️ هنا السبب
    RETURNING * INTO new_user_record;

    -- Update tenant counter if new row inserted
    IF FOUND THEN
        UPDATE public.tenants AS t
        SET current_users = t.current_users + 1,
            updated_at = now()
        WHERE t.id = p_tenant_id;
    END IF;

    -- Fetch existing user if not inserted
    IF new_user_record.id IS NULL THEN
        SELECT u.* INTO new_user_record
        FROM public.users AS u
        WHERE u.id = p_user_id; -- هنا كمان لازم نحدد الجدول
    END IF;

    -- Return record explicitly
    RETURN QUERY 
    SELECT 
        new_user_record.id,
        new_user_record.tenant_id,
        new_user_record.branch_id,
        new_user_record.email,
        new_user_record.role,
        new_user_record.name,
        new_user_record.phone,
        new_user_record.avatar,
        new_user_record.base_salary,
        new_user_record.allowances,
        new_user_record.deductions,
        new_user_record.work_schedule,
        new_user_record.is_active,
        new_user_record.created_at,
        new_user_record.updated_at;
END;
$$;


--
-- Name: generate_qr_session(uuid, integer); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.generate_qr_session(p_branch_id uuid, p_ttl_seconds integer, OUT out_session_id uuid, OUT out_token_hex text, OUT out_expires_at timestamp with time zone) RETURNS record
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
DECLARE
...
BEGIN
  ...
  v_tenant_id := v_branch_rec.tenant_id;

  -- check caller role + tenant
  SELECT u.role, u.tenant_id 
  INTO v_role, v_tenant_id
  FROM public.users u 
  WHERE u.id = v_caller_id;

  IF v_role NOT IN ('cad','admin') THEN
    RAISE EXCEPTION 'Not allowed to generate QR';
  END IF;

  IF v_tenant_id IS DISTINCT FROM v_branch_rec.tenant_id THEN
    RAISE EXCEPTION 'Branch does not belong to your tenant';
  END IF;

  -- create session token
  v_token_hex := encode(gen_random_bytes(32), 'hex');
  v_exp := now() + (p_ttl_seconds || ' seconds')::interval;

  INSERT INTO public.qr_sessions (
    branch_id, tenant_id, session_token, created_by, expires_at
  )
  VALUES (
    p_branch_id, v_branch_rec.tenant_id, v_token_hex, v_caller_id, v_exp
  )
  RETURNING id, session_token, expires_at 
  INTO out_session_id, out_token_hex, out_expires_at;
...
CREATE FUNCTION public.handle_new_user_profile() RETURNS trigger
    ...
    AS $$
BEGIN
  INSERT INTO public.users (id, tenant_id, email, role, name, is_active)
  VALUES (
    NEW.id,
    (SELECT id FROM public.tenants WHERE name = 'default-tenant' LIMIT 1),
    NEW.email,
    'employee',
    split_part(NEW.email, '@', 1), -- الاسم قبل الـ @
    TRUE
  );
  RETURN NEW;
END;
...
--
-- Name: increment_current_users(uuid); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.increment_current_users(p_tenant_id uuid) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
begin
  update tenants
...
--
-- Name: verify_qr_and_checkin(text, double precision, double precision, integer); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.verify_qr_and_checkin(p_token text, p_lat double precision, p_lng double precision, p_window_seconds integer DEFAULT 5) RETURNS TABLE(success boolean, message text)
    LANGUAGE plpgsql
    AS $$
DECLARE
  parts text[];
...
BEGIN
  ...
  IF array_length(parts,1) <> 3 THEN
    RETURN QUERY SELECT false, 'Invalid token format';
    RETURN;
  END IF;

  session_id := parts[1]::uuid;
  window_text := parts[2];
  mac_hex := lower(parts[3]);

  SELECT * INTO sess FROM public.qr_sessions WHERE id = session_id;
  IF NOT FOUND THEN
    RETURN QUERY SELECT false, 'Session not found or expired';
    RETURN;
  END IF;

  IF now() > sess.expires_at THEN
    RETURN QUERY SELECT false, 'Session expired';
    RETURN;
  END IF;

  -- compute expected mac over message "session_id|window"
  FOR i IN -1..1 LOOP
    expected_mac := hmac((session_id::text || '|' || (window_text::bigint + i)::text)::bytea, decode(sess.session_token,'hex'), 'sha256');
    expected_hex := encode(expected_mac, 'hex');
    IF expected_hex = mac_hex THEN
      allowed := true;
      EXIT;
    END IF;
  END LOOP;
  ...
  IF NOT allowed THEN
    RETURN QUERY SELECT false, 'QR code invalid or expired';
    RETURN;
  END IF;

  -- branch info
  SELECT * INTO branch_rec FROM public.branches WHERE id = sess.branch_id;
  IF NOT FOUND THEN
    RETURN QUERY SELECT false, 'Branch not found';
    RETURN;
  END IF;

  -- location check (haversine)
  lat1_rad := radians(p_lat);
  lat2_rad := radians(branch_rec.latitude::double precision);
  dlat := radians(branch_rec.latitude::double precision - p_lat);
  dlon := radians(branch_rec.longitude::double precision - p_lng);
  a := sin(dlat/2)^2 + cos(lat1_rad) * cos(lat2_rad) * sin(dlon/2)^2;
  c := 2 * atan2(sqrt(a), sqrt(1-a));
  dist := R * c;

  IF dist > branch_rec.radius_meters::double precision THEN
    RETURN QUERY SELECT false, 'You are not within branch radius';
    RETURN;
  END IF;

  -- prevent replay
  IF EXISTS (SELECT 1 FROM public.used_qr_tokens WHERE token = p_token) THEN
    RETURN QUERY SELECT false, 'QR token already used';
    RETURN;
  END IF;

  INSERT INTO public.used_qr_tokens(token) VALUES (p_token);

  -- verify tenant active/subscription
  IF (SELECT NOT is_active FROM public.tenants WHERE id = sess.tenant_id) THEN
    RETURN QUERY SELECT false, 'تم إيقاف العميل';
    RETURN;
  END IF;

  IF (SELECT subscription_end FROM public.tenants WHERE id = sess.tenant_id) IS NOT NULL
     AND now() > (SELECT subscription_end FROM public.tenants WHERE id = sess.tenant_id) THEN
    RETURN QUERY SELECT false, 'تم إيقاف العميل لانتهاء الاشتراك';
    RETURN;
  END IF;

  -- insert attendance (سجل check-in)
  INSERT INTO public.attendance (id, user_id, tenant_id, branch_id, method, check_in_time, created_at)
  VALUES (gen_random_uuid(), caller_id, sess.tenant_id, sess.branch_id, 'qr', now(), now());
...
--
-- Name: attendance_with_users; Type: VIEW; Schema: public; Owner: -
--

CREATE VIEW public.attendance_with_users AS
 SELECT a.id,
    a.tenant_id,
    a.user_id,
    a.branch_id,
...
--
-- Name: employee_salary_rules; Type: TABLE; Schema: public; Owner: -
--
...
--
-- Name: qr_sessions; Type: TABLE; Schema: public; Owner: -
--
...
--
-- Name: used_qr_tokens; Type: TABLE; Schema: public; Owner: -
--
...
--
-- Name: users_view_table; Type: TABLE; Schema: public; Owner: -
--
...
--
-- Name: billing_history billing_history_invoice_number_key; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: employee_salary_rules employee_salary_rules_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.employee_salary_rules
    ADD CONSTRAINT employee_salary_rules_pkey PRIMARY KEY (id);


--
-- Name: employee_salary_rules employee_salary_rules_user_id_rule_id_key; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.employee_salary_rules
    ADD CONSTRAINT employee_salary_rules_user_id_rule_id_key UNIQUE (user_id, rule_id);
...
--
-- Name: qr_sessions qr_sessions_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: used_qr_tokens used_qr_tokens_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.used_qr_tokens
    ADD CONSTRAINT used_qr_tokens_pkey PRIMARY KEY (token);
...
--
-- Name: users on_auth_user_created_profile; Type: TRIGGER; Schema: auth; Owner: -
--
...
--
-- Name: billing_history billing_history_tenant_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: employee_salary_rules employee_salary_rules_rule_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.employee_salary_rules
    ADD CONSTRAINT employee_salary_rules_rule_id_fkey FOREIGN KEY (rule_id) REFERENCES public.payroll_rules(id) ON DELETE CASCADE;


--
-- Name: employee_salary_rules employee_salary_rules_tenant_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.employee_salary_rules
    ADD CONSTRAINT employee_salary_rules_tenant_id_fkey FOREIGN KEY (tenant_id) REFERENCES public.tenants(id) ON DELETE CASCADE;


--
-- Name: employee_salary_rules employee_salary_rules_user_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.employee_salary_rules
    ADD CONSTRAINT employee_salary_rules_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.users(id) ON DELETE CASCADE;


--
-- Name: payroll_details payroll_details_payroll_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.payroll_details
    ADD CONSTRAINT payroll_details_payroll_id_fkey FOREIGN KEY (payroll_id) REFERENCES public.payroll(id) ON DELETE CASCADE;


--
-- Name: payroll_details payroll_details_tenant_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.payroll_details
    ADD CONSTRAINT payroll_details_tenant_id_fkey FOREIGN KEY (tenant_id) REFERENCES public.tenants(id) ON DELETE CASCADE;


--
-- Name: payroll_rules payroll_rules_tenant_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.payroll_rules
    ADD CONSTRAINT payroll_rules_tenant_id_fkey FOREIGN KEY (tenant_id) REFERENCES public.tenants(id) ON DELETE CASCADE;
...
--
-- Name: qr_sessions qr_sessions_branch_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.qr_sessions
    ADD CONSTRAINT qr_sessions_branch_id_fkey FOREIGN KEY (branch_id) REFERENCES public.branches(id);


--
-- Name: qr_sessions qr_sessions_tenant_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.qr_sessions
    ADD CONSTRAINT qr_sessions_tenant_id_fkey FOREIGN KEY (tenant_id) REFERENCES public.tenants(id);
...
--
-- Name: attendance Allow authenticated read access to attendance within tenant; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Allow authenticated read access to attendance within tenant" ON public.attendance FOR SELECT TO authenticated USING ((tenant_id = ( SELECT users.tenant_id
   FROM public.users
  WHERE (users.id = auth.uid()))));


--
-- Name: branches Allow authenticated read access to branches within tenant; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Allow authenticated read access to branches within tenant" ON public.branches FOR SELECT TO authenticated USING ((tenant_id = ( SELECT users.tenant_id
   FROM public.users
  WHERE (users.id = auth.uid()))));


--
-- Name: tenants Allow authenticated read access to own tenant; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Allow authenticated read access to own tenant" ON public.tenants FOR SELECT TO authenticated USING ((id = ( SELECT users.tenant_id
   FROM public.users
  WHERE (users.id = auth.uid()))));


--
-- Name: payroll Allow authenticated read access to payroll within tenant; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Allow authenticated read access to payroll within tenant" ON public.payroll FOR SELECT TO authenticated USING ((tenant_id = ( SELECT users.tenant_id
   FROM public.users
  WHERE (users.id = auth.uid()))));


--
-- Name: users Allow authenticated read access to users within tenant; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Allow authenticated read access to users within tenant" ON public.users FOR SELECT TO authenticated USING ((tenant_id = public.get_current_tenant_id()));


--
-- Name: branches Allow insert for authenticated users; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Allow insert for authenticated users" ON public.branches FOR INSERT TO authenticated WITH CHECK (true);


--
-- Name: billing_history Allow super_admin read billing history within tenant; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Allow super_admin read billing history within tenant" ON public.billing_history FOR SELECT TO authenticated USING (((tenant_id = ( SELECT users.tenant_id
   FROM public.users
...
--
-- Name: attendance Allow super_admin/cad manage attendance within tenant; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Allow super_admin/cad manage attendance within tenant" ON public.attendance TO authenticated USING (((tenant_id = ( SELECT users.tenant_id
   FROM public.users
  WHERE (users.id = auth.uid()))) AND (( SELECT users.role
   FROM public.users
  WHERE (users.id = auth.uid())) = ANY (ARRAY['super_admin'::text, 'cad'::text]))));


--
-- Name: branches Allow super_admin/cad manage branches within tenant; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Allow super_admin/cad manage branches within tenant" ON public.branches TO authenticated USING (((tenant_id = ( SELECT users.tenant_id
   FROM public.users
  WHERE (users.id = auth.uid()))) AND (( SELECT users.role
   FROM public.users
  WHERE (users.id = auth.uid())) = ANY (ARRAY['super_admin'::text, 'cad'::text]))));
...
--
-- Name: payroll Allow super_admin/cad manage payroll within tenant; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Allow super_admin/cad manage payroll within tenant" ON public.payroll TO authenticated USING (((tenant_id = ( SELECT users.tenant_id
   FROM public.users
  WHERE (users.id = auth.uid()))) AND (( SELECT users.role
   FROM public.users
  WHERE (users.id = auth.uid())) = ANY (ARRAY['super_admin'::text, 'cad'::text]))));
...
--
-- Name: users Allow super_admin/cad manage users within tenant; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Allow super_admin/cad manage users within tenant" ON public.users TO authenticated USING (((tenant_id = public.get_current_tenant_id()) AND (role = ANY (ARRAY['super_admin'::text, 'cad'::text]))));


--
-- Name: audit_logs Allow super_admin/cad read audit logs within tenant; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Allow super_admin/cad read audit logs within tenant" ON public.audit_logs FOR SELECT TO authenticated USING (((tenant_id = ( SELECT users.tenant_id
   FROM public.users
  WHERE (users.id = auth.uid()))) AND (( SELECT users.role
   FROM public.users
  WHERE (users.id = auth.uid())) = ANY (ARRAY['super_admin'::text, 'cad'::text]))));
...
--
-- Name: users Read own data; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Read own data" ON public.users FOR SELECT USING ((id = auth.uid()));
...
--
-- Name: attendance attendance open access; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "attendance open access" ON public.attendance USING (true) WITH CHECK (true);
...
CREATE POLICY tenants_insert_policy ON public.tenants FOR INSERT TO authenticated WITH CHECK (true);
...
--
-- Name: FUNCTION create_branch_and_update_tenant(branch_data jsonb); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.create_branch_and_update_tenant(branch_data jsonb) TO anon;
GRANT ALL ON FUNCTION public.create_branch_and_update_tenant(branch_data jsonb) TO authenticated;
GRANT ALL ON FUNCTION public.create_branch_and_update_tenant(branch_data jsonb) TO service_role;


--
-- Name: FUNCTION create_user_and_update_tenant(user_data jsonb); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.create_user_and_update_tenant(user_data jsonb) TO anon;
GRANT ALL ON FUNCTION public.create_user_and_update_tenant(user_data jsonb) TO authenticated;
GRANT ALL ON FUNCTION public.create_user_and_update_tenant(user_data jsonb) TO service_role;


--
-- Name: FUNCTION generate_qr_session(p_branch_id uuid, p_ttl_seconds integer, OUT out_session_id uuid, OUT out_token_hex text, OUT out_expires_at timestamp with time zone); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.generate_qr_session(p_branch_id uuid, p_ttl_seconds integer, OUT out_session_id uuid, OUT out_token_hex text, OUT out_expires_at timestamp with time zone) TO anon;
GRANT ALL ON FUNCTION public.generate_qr_session(p_branch_id uuid, p_ttl_seconds integer, OUT out_session_id uuid, OUT out_token_hex text, OUT out_expires_at timestamp with time zone) TO authenticated;
GRANT ALL ON FUNCTION public.generate_qr_session(p_branch_id uuid, p_ttl_seconds integer, OUT out_session_id uuid, OUT out_token_hex text, OUT out_expires_at timestamp with time zone) TO service_role;
...
GRANT ALL ON FUNCTION public.handle_new_user_profile() TO supabase_auth_admin;


--
-- Name: FUNCTION increment_current_users(p_tenant_id uuid); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.increment_current_users(p_tenant_id uuid) TO anon;
GRANT ALL ON FUNCTION public.increment_current_users(p_tenant_id uuid) TO authenticated;
GRANT ALL ON FUNCTION public.increment_current_users(p_tenant_id uuid) TO service_role;


--
-- Name: FUNCTION verify_qr_and_checkin(p_token text, p_lat double precision, p_lng double precision, p_window_seconds integer); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.verify_qr_and_checkin(p_token text, p_lat double precision, p_lng double precision, p_window_seconds integer) TO anon;
GRANT ALL ON FUNCTION public.verify_qr_and_checkin(p_token text, p_lat double precision, p_lng double precision, p_window_seconds integer) TO authenticated;
GRANT ALL ON FUNCTION public.verify_qr_and_checkin(p_token text, p_lat double precision, p_lng double precision, p_window_seconds integer) TO service_role;
...
--
-- Name: TABLE attendance_with_users; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.attendance_with_users TO anon;
GRANT ALL ON TABLE public.attendance_with_users TO authenticated;
GRANT ALL ON TABLE public.attendance_with_users TO service_role;
...
--
-- Name: TABLE billing_history; Type: ACL; Schema: public; Owner: -
--
...
--
-- Name: TABLE employee_salary_rules; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.employee_salary_rules TO anon;
GRANT ALL ON TABLE public.employee_salary_rules TO authenticated;
GRANT ALL ON TABLE public.employee_salary_rules TO service_role;
...
--
-- Name: TABLE payroll_rules; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.payroll_rules TO anon;
GRANT ALL ON TABLE public.payroll_rules TO authenticated;
GRANT ALL ON TABLE public.payroll_rules TO service_role;
...
--
-- Name: TABLE qr_sessions; Type: ACL; Schema: public; Owner: -
--
...
--
-- Name: TABLE used_qr_tokens; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.used_qr_tokens TO anon;
GRANT ALL ON TABLE public.used_qr_tokens TO authenticated;
GRANT ALL ON TABLE public.used_qr_tokens TO service_role;


--
-- Name: TABLE users_view; Type: ACL; Schema: public; Owner: -
--
...
--
-- Name: TABLE users_view_table; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.users_view_table TO anon;
GRANT ALL ON TABLE public.users_view_table TO authenticated;
GRANT ALL ON TABLE public.users_view_table TO service_role;
